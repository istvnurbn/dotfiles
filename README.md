...
> [!CAUTION]
> I'm fairly new to Nix. I did my best to make the configuration modular and follow the Dendritic pattern. However, that doesn't guarantee that it will work on other devices right away. Feel free to reach out to me or open a PR so I can make improvements.
...
**[`nix-settings.nix`](./modules/common/nix-settings.nix)**: Universal `nix` and `nixpkgs`.  
...
**[`nix-settings.nix`](./modules/darwin/nix-settings.nix)**: nix-darwin specific garbage collection settings.  
...
- [ ] Switch to `oxcl/nix-flake-helium-browser`
- [ ] Break out the `gaming` module to individual parts.
- [ ] Switch to `different-name/steam-config-nix`
